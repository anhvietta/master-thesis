...
from colbert import ColBERT_direct
...
def get_attention_mask(window_size=50, seq_len=max_length):
...
def main(
    ...
    with open(base_out + "out2.csv", "a") as file:
        file.write('Model size: {:.3f}MB\n'.format(get_model_size(model)))
    # Load the dataset
    dataset = ColBertDatasetAln_direct(train_file, train_matches, max_length=max_length, h5file=cached_train, padding=0)
    subset_start, subset_end = 0, 24000000
    selection = range(subset_start, subset_end+1)
    subset_size = 1000000 #int(len(dataset) * 0.9)
    test_subset_size = 100000 #len(dataset) - subset_size
    assert(subset_size + test_subset_size <= len(dataset))
    ...
    if test_subset_size > 0:
        unchosen = list(set(selection).difference(set(train_indices)))
        random.shuffle(unchosen)
        test_indices = unchosen[:test_subset_size]
        test_subset = Subset(dataset, test_indices)
        testloader = DataLoader(test_subset, batch_size=batch_size, shuffle=True, pin_memory=False)

        num_test_samples = 5
        test_samples = unchosen[:num_test_samples]

    '''testset = ColBertDataset(valid_file, valid_config, max_length=max_length, h5file=cached_valid)
    testloader = DataLoader(testset, batch_size=batch_size, shuffle=True, pin_memory=False)'''
    ...
    with open(base_out + "out2.csv", "a") as file:
        file.write(f"{model_parameters}\n")
    print("Epoch\tLoss\tUniqueAln\tSigAln\tScore\tTime")
    with open(base_out + "out2.csv", "a") as file:
        file.write("Epoch\tLoss\tUniqueAln\tSigAln\tScore\tTime\n")
    log_t = 0
    blur = 2.0
    end_blur = 0.1
    ...
    exp_name = f"experiment_{batch_size}_{learning_rate}_{model_parameters['dim']}_{model_parameters['conv_kernel']}_{model_parameters['n_layers']}_{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}"
    ...
    for epoch in range(trained_epoch, trained_epoch + epochs):
        ...
        t = time.time()
        rt = 0
        for batch_idx, (seq, seqlen, spanref, alnref, target_score, idx) in enumerate(dataloader):
            seq = seq.to(device)
            ...
            with autocast(device_type='cuda',dtype=torch.float16):  # Enable FP16
                tt = time.time()
                maps, blosum_mask = model(seq, None, seq_padding_mask, seqlen)
                ...
                loss, aln_loss, score_loss, score, matchloss, signalloss, simloss = model.loss_fn(
                    maps,
                    blosum_mask,
                    spanref,
                    alnref,
            ...
            if (epoch+1) % log_interval == 0:
                ...
                for i, m, rm in zip(idx, maps, ref_map):
                    if i in train_samples:
                        writer.add_image(f"Samples/sample_train_{i}/map", m[0,:,:].unsqueeze(0), global_step)
            ...
            if subset_size > 1000000 and global_step % ((num_batch // 10) if (num_batch // 10) > 0 else 1) == 0:
                ...
                writer.add_scalar("Loss/train_score", total_sloss / (batch_idx+1), global_step)
        ...
        if total_mloss / len(dataloader) < 0.9 and not use_scoring:
            use_scoring = True
            for name, p in model.colbert.named_parameters():
                if 'blosum' not in name:
                    p.register_hook(lambda grad: 0.1 * grad)

        print(f"{epoch + 1}/{trained_epoch + epochs}\t{total_loss / len(dataloader):.6f}\t{total_aloss/len(dataloader):.6f}\t{total_mloss/len(dataloader):.6f}\t{total_sloss/len(dataloader):.6f}\t{(time.time()-t)/3600:.2f}")
        with open(base_out + "out2.csv", "a") as file:
            file.write(f"{epoch + 1}/{trained_epoch + epochs}\t{total_loss / len(dataloader):.6f}\t{total_aloss/len(dataloader):.6f}\t{total_mloss/len(dataloader):.6f}\t{total_sloss/len(dataloader):.6f}\t{(time.time()-t)/3600:.2f}\n")
        ...
        if (epoch+1) % log_interval == 0:
            ...
            writer.add_scalar("Loss/train_score", total_sloss / len(dataloader), global_step)
            ...
            with torch.no_grad():
                for seq, seqlen, spanref, alnref, target_score, idx in testloader:
                    ...
                    with autocast(device_type='cuda',dtype=torch.float16):  # Enable FP16
                        ...
                        if (maps > 1.1).any() or (maps < -1.1).any():
                            ...
                            maps = torch.clamp(maps, min=-1, max=1)
                        alnref = alnref.to(device)
                        tloss, taln_loss, tscore_loss, score, tmloss, tsigloss, tsimloss = model.loss_fn(
                            maps,
                            blosum_mask,
                            spanref,
                            alnref,
                    ...
                    for i, m, rm in zip(idx, maps, ref_map):
                        if i in test_samples:
                            writer.add_image(f"Samples/sample_test_{i}/map", m[0,:,:].unsqueeze(0), global_step)
                ...
                writer.add_scalar("Loss/test_score", total_tsloss / len(testloader), global_step)
                ...
                print(f"Test\t{total_tloss / len(testloader):.6f}\t{total_taloss/len(testloader):.6f}\t{total_tmloss/len(testloader):.6f}\t{total_tsloss/len(testloader):.6f}\t0")
                with open(base_out + "out2.csv", "a") as file:
                    file.write(f"Test\t{total_tloss / len(testloader):.6f}\t{total_taloss/len(testloader):.6f}\t{total_tmloss/len(testloader):.6f}\t{total_tsloss/len(testloader):.6f}\t0\n")


if __name__ == "__main__":
    train_file = base_data + 'dataproc_direct.fasta'
    train_matches = base_data + 'dataproc_direct_matches.h5'
    cached_train = base_data + 'cached_direct.h5'
    valid_file = None
    valid_matches = None
    cached_valid = None
    batch_size = 500
    ...
    output_state = base_ckpts + 'ckpts_colbert+direct_conv+relpe+match+gttl_1m_48_7_5'
    save_interval = 3
    log_interval = 3
    checkpoint = None #base_ckpts + 'ckpts_colbert+direct_convstack+relpe+match+gttl_1m_128_7_3_24.pth'
    stage_switch = 20
