...
def main(
    ...
    subset_size = 120000000 #int(len(dataset) * 0.9)
    ...
    test_subset_size = 12000000 #len(dataset) - subset_size
    assert(len(testset) >= test_subset_size)
    ...
    exp_name = f"{datetime.datetime.now().strftime('%m%d-%H%M')}_{batch_size}_{learning_rate}_{temp}_{get_model_info_str(model_parameters)}"
    ...
    for epoch in range(trained_epoch, trained_epoch + epochs):
        ...
        for batch_idx, (seq, seqlen, spanref, alnref, _, idx) in enumerate(dataloader):
            ...
            with autocast(device_type='cuda',dtype=torch.float16):  # Enable FP16
                tt = time.time()
                maps = model(seq, seq_padding_mask, num_enc_used)
            ...
            if subset_size >= 1000000 and global_step % ((num_batch // 20) if (num_batch // 20) > 0 else 1) == 0:
                if not log_dir:
                    log_dir = base_out + exp_path
                    writer = SummaryWriter(log_dir)
                writer.add_scalar("Loss/train_total", loss.item(),  global_step)
                writer.add_scalar("Loss/train_aln", aln_loss.item(), global_step)
                writer.add_scalar("Loss/train_match", matchloss.item(), global_step)
                writer.add_scalar("Loss/train_mmatch", mmatchloss.item(), global_step)
                writer.add_scalar("Loss/train_sigloss", signalloss.item(), global_step)
                writer.add_scalar("Loss/train_simloss", simloss.item() , global_step)
                writer.add_scalar("Hyperparameter/LR", scheduler.get_last_lr()[0], global_step)
                token_emb = model.tokenizer.get_embedding().detach().cpu()
                ...
                torch.save(
                    token_emb,
                    output_prefix + f"_tokenizer_{global_step}.pth"
                )
...
if __name__ == "__main__":
    ...
    output_state = base_ckpts + 'ckpts_pyramid_ptwise_noskip_120m_0.07_32+64+96+128+160+192_3+3+5+5+7+9_1+2+2+3+4+5_128_1+3+5'
    save_interval = 1
    log_interval = 1
    checkpoint = None
    ...
    #model parameters
    model_parameters = {
        'dims': [[32, 64, 96, 128, 160, 192]],
        'conv_kernels':[[3, 3, 5, 5, 7, 9]],
        'dilations': [[1, 2, 2, 3, 4, 5]],
        'enc_type': 0,
