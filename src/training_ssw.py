...
def main(
    ...
    logging('Model size: {:.3f}MB'.format(get_model_size(model)))
    # Load the dataset
    ...
    subset_size = 800 #int(len(dataset) * 0.9)
    ...
    testset = ColBertDatasetAln_direct(valid_file, valid_matches, max_length=max_length, h5file=cached_valid, padding=0)
    testsubset_start, testsubset_end = 0, len(dataset) - 1
    test_subset_size = 800 #len(dataset) - subset_size
    ...
    max_scale = 1
    end_mscale = 1
    max_scale_f = get_decay_factor(max_scale, end_mscale, 5)
    ...
    for epoch in range(trained_epoch, trained_epoch + epochs):
        ...
        for batch_idx, (seq, seqlen, spanref, alnref, _, idx) in enumerate(dataloader):
            ...
            with autocast(device_type='cuda',dtype=torch.float16):  # Enable FP16
                tt = time.time()
                maps, traceback = model(seq, seq_padding_mask, num_enc_used)
                logging(f"{traceback.min()}, {traceback.max()}, {traceback.shape}")
            ...
            if (epoch+1) % log_interval == 0:
                ...
                for i, m, rm,tb in zip(idx, maps, ref_map, traceback):
            ...
            if torch.isnan(loss).any():
                logging("NaN detected in logits!")
            loss = loss / grad_acc_iter
            scaler.scale(loss).backward()
            if ((batch_idx + 1) % grad_acc_iter == 0) or (batch_idx + 1 == len(dataloader)):
            ...
            for name, param in model.named_parameters():
                if torch.isnan(param).any():
                    logging(f"NaN in weights: {name}")

        logging(f"{epoch + 1}/{trained_epoch + epochs}\t{total_loss / len(dataloader):.6f}\t{total_aloss/len(dataloader):.6f}\t{total_mloss/len(dataloader):.6f}\t{(time.time()-t)/3600:.2f}")
        ...
        if (epoch+1) % log_interval == 0:
            ...
            with torch.no_grad():
                for seq, seqlen, spanref, alnref, target_score, idx in testloader:
                    ...
                    with autocast(device_type='cuda',dtype=torch.float16):  # Enable FP16
                        ...
                        if (maps > 1.1).any() or (maps < -1.1).any():
                            ...
                            maps, traceback = torch.clamp(maps, min=-1, max=1)
                    ...
                    for i, m, rm, tb in zip(idx, maps, ref_map, traceback):
                        if i in test_samples:
                            ...
                            writer.add_image(f"Samples/sample_test_{i}/traceback", ((tb-tb.min())/(tb.max()-tb.min())).unsqueeze(0), global_step)
                ...
                logging(f"Test\t{total_tloss / len(testloader):.6f}\t{total_taloss/len(testloader):.6f}\t{total_tmloss/len(testloader):.6f}\t0")

        if num_enc_used < model.num_enc and total_mloss / len(dataloader) / num_enc_used < 1.:
            num_enc_used += 1
...
if __name__ == "__main__":
    ...
    epochs = 2
    learning_rate = 0.004
    output_state = base_ckpts + 'ckpts_ssw_test_32+96+160_7+9+11_1+2+3_128'
    save_interval = 1
    log_interval = 1
    checkpoint = None
