"""
compute precision/recall of MMseqs2 against a SW reference
- ss_file: result file generated by running MMseqs2
...
with open(ss_file, 'r') as handler:
    ss_data = [line[:-1].split("\t") for line in list(handler)]

with open(sw_file, 'r') as handler:
    sw_data = [line[:-1].split("\t") for line in list(handler)]

ss_seqpair_set = set((e[1], e[0]) for e in ss_data)
